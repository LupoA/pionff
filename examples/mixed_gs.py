...
from pionff.formfactors.gounaris_sakurai import argFpi, absFpi, gs_amu
import matplotlib.pyplot as plt
import json


def hp_GS(L, m_muon, m_pi, m_rho, g_ppr, x0min, x0max):
    assert x0min == 0
...
def slide_x0min():
    """
    combines mll and hp matching at different values of x0cut
    """
...
def run():
    ...
    result_from_mll, error_from_mll = mll_GS(
        Lref_fm * gev_fm_conversion,
        mass_muon_GeV,
        mass_pi0_GeV,
        mass_rho0_GeV,
        ...
        x0max=x0_max * gev_fm_conversion,
    )

    result_from_hp, error_from_hp = hp_GS(
        Lref_fm * gev_fm_conversion,
        mass_muon_GeV,
        mass_pi0_GeV,
        mass_rho0_GeV,
        g_ppr_0,
        x0min=x0_min * gev_fm_conversion,
