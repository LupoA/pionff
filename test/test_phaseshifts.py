...
def test_phases():
    delta_at_rho = 57.2958 * gkpry_phase(
        mass_rho0_GeV, m_pi=mass_pi0_GeV, m_rho=mass_rho0_GeV
    )
    assert delta_at_rho - 90 < 1e-4, "Failed testing delta(M_rho) for GKPRY."

    delta_at_rho = 57.2958 * gs_phase(
        mass_rho0_GeV, m_pi=mass_pi0_GeV, m_rho=mass_rho0_GeV, g_ppr=g_ppr_0
    )
    assert delta_at_rho - 90 < 1e-4, "Failed testing delta(M_rho) for GS."

    assert (
        gkpry_phase(2 * mass_pi0_GeV, m_pi=mass_pi0_GeV, m_rho=mass_rho0_GeV) == 0
